...
Suit Card::get_suit(Suit trump) const {
  Suit leftBower = Suit_next(trump);
  if (rank == JACK && suit == leftBower) {
    return trump;
  }
  return suit;
}
...
//  if(trump == DIAMONDS && suit == HEARTS && rank == JACK){
//      return trump;
//  }
//  else if(trump == HEARTS && suit == DIAMONDS && rank == JACK){
//    return trump;
//  }
...
bool operator<(const Card &lhs, const Card &rhs){
 if(lhs.get_rank() < rhs.get_rank()){
   ...
   else if (rhs.get_suit() == SPADES) {
    return false;
   }
   else if (lhs.get_suit() == CLUBS && rhs.get_suit() == HEARTS){
     return false;
   }
   else {
     return true;
